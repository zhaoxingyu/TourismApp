...
import com.nostra13.universalimageloader.cache.disc.DiskCacheFolder;
...
final class LoadAndDisplayImageTask implements Runnable, com.nostra13.universalimageloader.utils.IoUtils.CopyListener {
	...
	private static final String LOG_LOAD_IMAGE_FROM_NATIVE_MEMORY_CACHE = "Load image from native memory cache [%s]";
	...
	private static final String LOG_TASK_SPENDTIME = "Task was completed, spend [%s] ms";
	private static final String LOG_TASK_AVGTIME = "Tasks total: [%s], avg spend [%s] ms";
	...
	public void run() {
        ...
        totalSpendTime += runningTime;
        totalCounts++;
        L.d(LOG_TASK_SPENDTIME, runningTime + "");
        runningTime = totalSpendTime / totalCounts;
        L.d(LOG_TASK_AVGTIME, totalCounts + "", runningTime + "");
    }
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		Bitmap bitmap = null;
		try {
            if (options.isCacheInNativeMemory()) {
                InputStream is = configuration.nativeMemoryCache.get(uri);
                if (is != null) {
                    L.d(LOG_LOAD_IMAGE_FROM_NATIVE_MEMORY_CACHE, memoryCacheKey);
                    loadedFrom = LoadedFrom.NATIVE_MEMORY_CACHE;
                    
                    bitmap = decodeImage(uri, is);
//                    bitmap = BitmapFactory.decodeStream(is);
		    ...
		    if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
		        ...
		        if (options.isCacheInNativeMemory() && bitmap != null) {
	                configuration.nativeMemoryCache.put(uri, new FileInputStream(imageFile));
	            }
		    }
	...
	private Bitmap decodeImage(String imageUri) throws IOException {
	    return decodeImage(imageUri, null);
	}
	
    private Bitmap decodeImage(String imageUri, InputStream is) throws IOException {
        ViewScaleType viewScaleType = imageAware.getScaleType();
        ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, imageUri, uri, targetSize, viewScaleType, getDownloader(),
                options, is);
	...
	private boolean downloadImage() throws IOException {
		...
		boolean succ = configuration.diskCache.save(uri, is, this);
		if (succ) {
		    configuration.diskCache.sharkFolder((DiskCacheFolder) options.getExtraForDownloader());
        }
		return succ;
	}
